...
import { runMarketplaceVersionSync } from "@/lib/marketplace/version-sync";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";
export const maxDuration = 60;

const ENDPOINT = "/api/cron/marketplace-version-sync";
...
export async function GET(req: NextRequest) {
  ...
  const result = await runMarketplaceVersionSync({
    limit: intFromEnv("LUMO_MARKETPLACE_VERSION_SYNC_LIMIT", 200),
  });
  await recordCronRun({
    endpoint: ENDPOINT,
