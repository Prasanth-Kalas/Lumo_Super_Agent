...
import { runAgentHealthMonitor } from "@/lib/trust/health-monitor";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";
export const maxDuration = 60;

const ENDPOINT = "/api/cron/agent-health-monitor";
...
export async function GET(req: NextRequest) {
  ...
  const result = await runAgentHealthMonitor({
    limit: intFromEnv("LUMO_AGENT_HEALTH_MONITOR_LIMIT", 500),
  });
  await recordCronRun({
    endpoint: ENDPOINT,
