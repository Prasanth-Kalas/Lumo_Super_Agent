 ...
 * absolute path). Anything else falls back to `/`. Supabase preserves
 * the redirectTo query string when it bounces the user back through
 * the provider, so any `?next=` we set here lands on
