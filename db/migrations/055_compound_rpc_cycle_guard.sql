-- COMPOUND-RPC-CYCLE-GUARD-1 RPC hardening.
--
-- Rollback:
--   Re-apply db/migrations/048_compound_idempotency_conflict.sql to restore
--   the prior RPC body without the DB-side dependency-cycle guard.
--
...
--
-- Cycle-check doctrine:
--   JS-side validation via replayCompoundTransaction is the primary high-level
--   cycle check. This DB guard catches any service-role caller that bypasses
--   the JS layer before it can write cyclic dependency edges.
...
begin
  ...
  -- Defense in depth: reject transitive dependency cycles before any ledger
  -- rows are inserted, even if a future service-role caller skips the JS
  -- replayCompoundTransaction validation path.
  with recursive dependency_edges as (
    select
      nullif(edge.value->>'dependency_client_leg_id', '') as dependency_client_leg_id,
      nullif(edge.value->>'dependent_client_leg_id', '') as dependent_client_leg_id
    from jsonb_array_elements(coalesce(payload->'dependencies', '[]'::jsonb)) as edge(value)
    where nullif(edge.value->>'dependency_client_leg_id', '') is not null
      and nullif(edge.value->>'dependent_client_leg_id', '') is not null
  ),
  dependency_walk as (
    select
      dependency_client_leg_id as root_client_leg_id,
      dependent_client_leg_id as current_client_leg_id,
      array[dependency_client_leg_id, dependent_client_leg_id]::text[] as path,
      dependency_client_leg_id = dependent_client_leg_id as has_cycle
    from dependency_edges

    union all

    select
      dependency_walk.root_client_leg_id,
      dependency_edges.dependent_client_leg_id,
      dependency_walk.path || dependency_edges.dependent_client_leg_id,
      dependency_edges.dependent_client_leg_id = any(dependency_walk.path)
    from dependency_walk
    join dependency_edges
      on dependency_edges.dependency_client_leg_id = dependency_walk.current_client_leg_id
    where not dependency_walk.has_cycle
  )
  select
    path[array_length(path, 1) - 1] as dependency_client_leg_id,
    path[array_length(path, 1)] as dependent_client_leg_id,
    path
    into v_cycle
    from dependency_walk
   where has_cycle
  ...
  if found then
    raise exception 'INVALID_COMPOUND_DEPENDENCY_GRAPH_CYCLE'
      using hint =
        'offending_edge=' || v_cycle.dependency_client_leg_id || '->' || v_cycle.dependent_client_leg_id ||
        '; cycle=' || array_to_string(v_cycle.path, '->');
  end if;
