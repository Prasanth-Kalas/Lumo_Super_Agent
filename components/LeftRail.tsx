...
export default function LeftRail({ onNewChat, currentSessionId }: LeftRailProps) {
  ...
  return (
    <aside className="hidden lg:flex h-full w-[240px] shrink-0 flex-col border-r border-lumo-hair bg-lumo-bg">
      {/* Brand header */}
      <div className="px-4 pt-4 pb-3 border-b border-lumo-hair">
        <Link href="/" className="flex items-center gap-2.5 group">
          <span className="relative inline-flex items-center justify-center">
            <BrandMark size={22} className="text-lumo-fg" />
            <span className="absolute inset-0 rounded-full bg-lumo-accent/20 blur-md opacity-60 group-hover:opacity-100 transition-opacity" />
          </span>
          <div className="flex flex-col leading-tight">
            <span className="text-[14px] font-semibold tracking-tight text-lumo-fg">
              Lumo
            </span>
            <span className="text-[10px] uppercase tracking-[0.14em] text-lumo-fg-low">
              one app · any task
            </span>
          </div>
        </Link>
      ...
      <div className="p-3">
        <button
          type="button"
          onClick={onNewChat}
          className="w-full h-9 rounded-lg bg-lumo-accent text-lumo-accent-ink text-[13px] font-medium hover:brightness-110 transition relative overflow-hidden group"
        >
          ...
          <span className="absolute inset-0 bg-[radial-gradient(circle_at_30%_50%,rgba(255,255,255,0.25),transparent_60%)] opacity-0 group-hover:opacity-100 transition-opacity" />
        </button>
      </div>
      ...
      <div className="px-3 pt-1 pb-2">
        ...
        <ul className="mt-1.5 space-y-0.5">
          {recents.length === 0 ? (
            <li className="px-2 py-1.5 text-[11.5px] text-lumo-fg-low">
              ...
              return (
                <li key={s.session_id}>
                  <Link
                    ...
                    className={
                      "block rounded-md px-2 py-1.5 text-[12.5px] truncate transition-colors " +
                      (isActive
                  ...
                  >
                    ...
                    {s.trip_count > 0 ? (
                      <span className="ml-1.5 inline-block text-[10px] text-lumo-accent align-middle">
      ...
      {/* Agents */}
      <div className="px-3 pt-2 pb-2 mt-1 border-t border-lumo-hair">
        <SectionHeader>Agents</SectionHeader>
        <ul className="mt-1.5 space-y-0.5">
          {agents.map((a) => (
            <li
              key={a.agent_id}
              className="flex items-center gap-2 px-2 py-1.5 rounded-md text-[12.5px] text-lumo-fg-mid hover:bg-lumo-elevated/60 hover:text-lumo-fg transition-colors"
              title={`${a.display_name} — ${a.health}`}
            >
              <span className="w-4 text-center text-[13px] text-lumo-accent opacity-90">
                {a.icon}
              </span>
      ...
      {/* Footer nav */}
      <div className="mt-auto border-t border-lumo-hair p-3 space-y-0.5">
...
function SectionHeader({ children }: { children: React.ReactNode }) {
  return (
    <div className="px-2 pt-2 text-[10px] uppercase tracking-[0.14em] text-lumo-fg-low font-medium">
      {children}
    </div>
  );
}
...
function FooterLink({ href, label }: { href: string; label: string }) {
  return (
    <Link
      href={href}
      className="block rounded-md px-2 py-1.5 text-[12px] text-lumo-fg-low hover:text-lumo-fg hover:bg-lumo-elevated/60 transition-colors"
    >
      {label}
    </Link>
  );
